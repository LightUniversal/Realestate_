...
const LoginScreen = () => {
  ...
  return (
    <div className=' bg-black top-10 relative'>
      <Form className=' text-white md:w-3/5 w-4/5 mx-auto border border-slate-800 rounded-2xl shadow-md px-10 py-10' onSubmit={ (e) => {submitHandler(e)}}>
        <div className="email my-3">
            ...
            <input type="email" name="email" id="email" className=' w-full h-16 border-none rounded outline-none text-slate-200 bg-slate-800 px-5' placeholder = "email address" value={email} onChange={(e) => {setEmail(e.target.value)}} />
        </div>
        <div className="password my-5">
            ...
            <input type="password" name="password" id="password" className='w-full border-none outline-none h-16 rounded text-slate-200 bg-slate-800 shadow-sm px-5' placeholder = "password" value={password} onChange={(e) => { setPassword(e.target.value)}} />
        </div>
